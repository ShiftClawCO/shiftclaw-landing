...
- `spec/project.md` — Architecture, conventions, tech decisions (project constitution)
- `spec/prd/<feature-slug>/prd.md` — PRD (12-section architecture spec)
- `spec/prd/<feature-slug>/<N>/` — Per-issue workspace: spec.md (issue snapshot), tasks.md (implementation checklist), notes.md (agent notes)

PRDs are generated by Whale before issue decomposition. Ink reads them + the project constitution to generate concrete implementation tasks. If a standalone issue has no PRD, Ink generates one autonomously before implementing.
